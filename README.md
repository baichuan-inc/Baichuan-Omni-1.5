...
- ** Openmm-Medical 中的图像来自 42 个公开的医学图像数据集，例如 ACRIMA（眼底图像）、BioMediTech（显微镜图像）和 CoronaHack（X 射线）。**
- ** Openmm-Medical 总共包含 88,996 张图像，为方便评估不同的大模型，每张图像都设计为多项选择题的形式。**
- ** 为促进全模态大模型在医疗领域的发展，我们即将开源该评估数据集。**
